...
interface Props {
  onSubmit: (url: string, competitors: string[], htmlInput?: { html: string; baseUrl?: string }) => void;
  loading: boolean;
}
...
export function AuditForm({ onSubmit, loading }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (inputMode === 'html') {
      if (htmlCode.trim().length < 20) return;
      const finalBase = baseUrl.trim() ? normalizeUrl(baseUrl) : undefined;
      onSubmit(finalBase || 'html-paste://local', [], { html: htmlCode, baseUrl: finalBase });
    } else {
          ...
          <div className="space-y-3 text-left">
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3">
              ...
              <input
                ...
                placeholder="e.g., https://example.com/page-being-audited"
              ...
              <p className="mt-1.5 text-xs text-amber-600">
                Providing the page URL helps resolve relative links and gives more accurate results for off-site and branding checks.
              </p>
            </div>
            <button
              type="submit"
            ...
            >
              {loading ? 'Auditing...' : 'Audit HTML Code'}
            </button>
          </div>
        )}
      </form>
